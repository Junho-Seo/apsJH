...
print(bubbles)  # [1, 2, 2, 3, 3, 3, 4, 5, 5, 7, 9]
...
print(temp)  # [1, 2, 2, 3, 3, 3, 4, 5, 5, 7, 9]
...
# 학습 시 문제 난이도 결정하기 or 다른 사람의 코드를 언제 봐야할까?
# 문제를 열었다
# 1. 설계가 불가능 (감도 안온다, 손도 못대겠다) -> 레벨 부족
#   - 이론 공부 + 기본 코드부터 (구글링 + gpt)
#   - 레벨을 낮춰 진행하는 것도 방법
# 2. 설계가 가능하다.
#   - 시간을 많이 쓰더라도 도전(최대 3시간)
#   - 현재는 문제를 많이 풀어보는게 중요하므로 시간을 너무 오래 쓰지 않는다
#       - 3시간동안 풀었는데 진전이 있다? 계속 진행
#       - 3시간동안 풀었는데 진전이 없는경우 -> 다른 사람의 정답 코드를 참고
#       - 핵심은 접근법을 가져와 다시 직접 풀어보는 것. 단순 복붙은 금물.
#          - 내 코드 다 지우기 + 다른 사람 설명 및 코드 참고 + 안 보고 처음부터 구현
