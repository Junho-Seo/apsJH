...
n = int(input())        # 높이 측정값의 개수 입력받기
road = list(map(int, input().split()))       # 높이 측정값들을 리스트로 입력받기

max_uphill = 0  # 가장 큰 오르막길의 크기를 저장할 변수
current_uphill = 0  # 현재 오르막길의 크기를 추적할 변수

for i in range(n-1):  # 높이 리스트를 순회하며 오르막길 계산
    if road[i] < road[i+1]:  # 현재 높이가 다음 높이보다 낮으면
        current_uphill += road[i+1] - road[i]  # 오르막길 크기 추가
    else:  # 오르막길이 끝나는 경우
        if current_uphill > max_uphill:  # 현재 오르막길이 지금까지의 최대 오르막길보다 크다면
            max_uphill = current_uphill  # 최대 오르막길 크기 갱신
        current_uphill = 0  # 현재 오르막길 크기 초기화

# 모든 높이를 다 확인한 후 마지막 오르막길이 남아있는지 확인
if current_uphill > max_uphill:
    max_uphill = current_uphill

print(max_uphill)  # 가장 큰 오르막길의 크기 출력