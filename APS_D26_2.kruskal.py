...
for _ in range(E):
    ...
    edge.append([u, v, w])  # 출발, 도착, 가중치 묶어서 간선 정보에 저장
edge.sort(key=lambda x : x[2])  # 가중치 기준으로 오름차순 정렬
parents = [i for i in range(V)]       # 대표원소 배열 make-set
...
# MST의 간선수 N = 정점 수 - 1
cnt = 0     # 선택한 edge의 수 (사용 이유: N-1 가 되면 신장트리 완성 -> break, 시간 효율을 위해)
total = 0   # MST 가중치의 합
# print(edge)
for u, v, w in edge:
    # 출발점과 도착점이 같은 그룹에 속해있다면, 이미 연결된 친구들이다.
    # 다른 집합이라면 (== 싸이클이 없다면)
    if find_set(u) != find_set(v):  # 싸이클이 없다면 (같으면 싸이클)
